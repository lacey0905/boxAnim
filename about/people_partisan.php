<!DOCTYPE html>
<html lang="ko">
<head>
    <?php include "../include/_head_common.php"; ?>

    <!-- 소개 스타일 //-->
    <link rel="stylesheet" href="../css/about.css">
    <!--//-->
</head>
<body>
    <div id="wrap">

        <?php include "../include/_body_skip.php"; ?>

        <?php include "../include/_body_header.php"; ?>

        <div id="container">

            <!-- location //-->
            <div id="loc">
                <ul>
                    <li class="home"><a href="">홈</a></li>
                    <li><span>소개</span></li>
                    <li><span>사람들</span></li>
                    <li><strong>국회의원</strong></li>
                </ul>
                <?php include "../include/_loc_option_menu.php"; ?>
            </div>
            <!--//-->

            <div id="contents">
                <section class="partisan-wrap layout-1280">
                    <div class="page-title">
                        <h2>국회의원</h2>
                        <p>
                            국민과 소통하며 일하는 일꾼이 되겠습니다.
                        </p>
                    </div>

                    <div class="sch-tab-cate mt50">
                        <ul>
                            <li class="active"><a href="">지역구</a></li>
                            <li><a href="">비례대표</a></li>
                        </ul>
                    </div>

                    <div class="sch-area-name mt40">
                        <ul>
                            <li class="active"><a href="">서울</a></li>
                            <li><a href="">부산</a></li>
                            <li><a href="">대구</a></li>
                            <li><a href="">인천</a></li>
                            <li><a href="">광주</a></li>
                            <li><a href="">대전</a></li>
                            <li><a href="">울산</a></li>
                            <li><a href="">세종</a></li>
                            <li><a href="">경기</a></li>
                            <li><a href="">강원</a></li>
                            <li><a href="">충북</a></li>
                            <li><a href="">충남</a></li>
                            <li><a href="">전북</a></li>
                            <li><a href="">전남</a></li>
                            <li><a href="">경북</a></li>
                            <li><a href="">경남</a></li>
                            <li><a href="">제주</a></li>
                        </ul>
                    </div>

                    <!-- 리스트 //-->
                    <p class="txt-right mt50">* 사진, 이름을 클릭하면 자세한 정보를 보실 수 있습니다.</p>
                    <div class="result-search row mt20">                        
                        <ul>
                            <li>
                                <div class="thumb">
                                    <a href="javascript:peoplePop('peoplePopup','table','1');"><img src="../img/noimg_logo.png" alt="홍길동"></a>
                                </div>
                                <div class="info">
                                    <dl>
                                        <dt>
                                            <h3><a href="javascript:peoplePop('peoplePopup','table','1');">홍길동</a></h3>
                                            <span>종로구</span>
                                        </dt>
                                        <dd class="row">
                                            <p>
                                                <span>생년월일</span>
                                                1960-12-10
                                            </p>
                                            <p>
                                                <span>전화</span>
                                                000-000-0000
                                            </p>
                                            <p>
                                                <span>이메일</span>
                                                sangtoil@hanmail.net
                                            </p>
                                        </dd>
                                    </dl>
                                    <div class="sns">
                                        <a href="" class="fa">페이스북</a>
                                        <a href="" class="bl">블로그</a>
                                        <a href="" class="in">인스타</a>
                                        <a href="" class="yu">유투브</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="thumb">
                                    <a href="javascript:peoplePop('peoplePopup','table','2');"><img src="../img/noimg_logo.png" alt="홍길동"></a>
                                </div>
                                <div class="info">
                                    <dl>
                                        <dt>
                                            <h3><a href="javascript:peoplePop('peoplePopup','table','2');">홍길동</a></h3>
                                            <span>종로구</span>
                                        </dt>
                                        <dd class="row">
                                            <p>
                                                <span>생년월일</span>
                                                1960-12-10
                                            </p>
                                            <p>
                                                <span>전화</span>
                                                000-000-0000
                                            </p>
                                            <p>
                                                <span>이메일</span>
                                                sangtoil@hanmail.net
                                            </p>
                                        </dd>
                                    </dl>
                                    <div class="sns">
                                        <a href="" class="fa">페이스북</a>
                                        <a href="" class="bl">블로그</a>
                                        <a href="" class="in">인스타</a>
                                        <a href="" class="yu">유투브</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="thumb">
                                    <a href="javascript:peoplePop('peoplePopup','table','3');"><img src="../img/noimg_logo.png" alt="홍길동"></a>
                                </div>
                                <div class="info">
                                    <dl>
                                        <dt>
                                            <h3><a href="javascript:peoplePop('peoplePopup','table','3');">홍길동</a></h3>
                                            <span>종로구</span>
                                        </dt>
                                        <dd class="row">
                                            <p>
                                                <span>생년월일</span>
                                                1960-12-10
                                            </p>
                                            <p>
                                                <span>전화</span>
                                                000-000-0000
                                            </p>
                                            <p>
                                                <span>이메일</span>
                                                sangtoil@hanmail.net
                                            </p>
                                        </dd>
                                    </dl>
                                    <div class="sns">
                                        <a href="" class="fa">페이스북</a>
                                        <a href="" class="bl">블로그</a>
                                        <a href="" class="in">인스타</a>
                                        <a href="" class="yu">유투브</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="thumb">
                                    <a href="javascript:peoplePop('peoplePopup','table','4');"><img src="../img/noimg_logo.png" alt="홍길동"></a>
                                </div>
                                <div class="info">
                                    <dl>
                                        <dt>
                                            <h3><a href="javascript:peoplePop('peoplePopup','table','4');">홍길동</a></h3>
                                            <span>종로구</span>
                                        </dt>
                                        <dd class="row">
                                            <p>
                                                <span>생년월일</span>
                                                1960-12-10
                                            </p>
                                            <p>
                                                <span>전화</span>
                                                000-000-0000
                                            </p>
                                            <p>
                                                <span>이메일</span>
                                                sangtoil@hanmail.net
                                            </p>
                                        </dd>
                                    </dl>
                                    <div class="sns">
                                        <a href="" class="fa">페이스북</a>
                                        <a href="" class="bl">블로그</a>
                                        <a href="" class="in">인스타</a>
                                        <a href="" class="yu">유투브</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div class="thumb">
                                    <a href="javascript:peoplePop('peoplePopup','table','5');"><img src="../img/noimg_logo.png" alt="홍길동"></a>
                                </div>
                                <div class="info">
                                    <dl>
                                        <dt>
                                            <h3><a href="javascript:peoplePop('peoplePopup','table','5');">홍길동</a></h3>
                                            <span>종로구</span>
                                        </dt>
                                        <dd class="row">
                                            <p>
                                                <span>생년월일</span>
                                                1960-12-10
                                            </p>
                                            <p>
                                                <span>전화</span>
                                                000-000-0000
                                            </p>
                                            <p>
                                                <span>이메일</span>
                                                sangtoil@hanmail.net
                                            </p>
                                        </dd>
                                    </dl>
                                    <div class="sns">
                                        <a href="" class="fa">페이스북</a>
                                        <a href="" class="bl">블로그</a>
                                        <a href="" class="in">인스타</a>
                                        <a href="" class="yu">유투브</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <!--//-->

                </section>
            </div>

        </div>

        <?php include "../include/_body_footer.php"; ?>

    </div>
</body>
</html>